...
> **难度** ⭐⭐☆☆☆ · **前置**：[4.1 Python 基础](1-4-1-python.md)、[3.1 线性代数](1-3-1-linear-algebra.md)
...
最后说一句你以后会反复用到的事实：PyTorch 里的**张量（tensor）**，几乎就是"能跑在 GPU 上、还能自动求导的 NumPy 数组"。它们的 API 高度相似——`shape`、广播、`@` 矩阵乘法都通用，还能用 `torch.from_numpy()` 互相转换。所以你现在学的每一个 NumPy 操作，到了 PyTorch 里几乎都能原样照搬（见 [2.4 PyTorch 基础](../ch2-round1/2-2-4-pytorch-basics.md)）。
